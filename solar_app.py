...
# --- CONFIGURARE PAGINA ---
st.set_page_config(page_title="Arhitect Solar Planner v1", layout="wide")
st.title("🌱 Arhitect Solar Planner: Layout Dinamic")

# --- SIDEBAR: SETARI MANUALE ---
...
# --- FUNCTIA DINAMICA ---
def genereaza_plan_tehnic_dinamic(L, l, d, d_rand, d_teh=0.3, d_acc=1.2, max_t=150, pos_echip="dreapta-sus"):
    fig, ax = plt.subplots(figsize=(16, 8))
    ax.set_xlim(-1, L+1)
    ax.set_ylim(-1, l+1)
    ax.add_patch(patches.Rectangle((0,0), L, l, linewidth=3, edgecolor='black', facecolor='#fdfdfd', zorder=1))

    # --- Pozitionare echipamente fixe in colt ---
    ...
    # Germinare
    ax.add_patch(patches.Rectangle((ibc_x-1 if pos_echip.startswith("dreapta") else ibc_x+1, germ_y), 2, 3, color='#e67e22', alpha=0.2, zorder=2))
    # Tablou
    ax.plot(tablou_x, tablou_y, 'rs', markersize=12, zorder=10)

    # --- Calcul numar coloane/randuri dinamice ---
    available_width = L - 2  # lasam margini
    available_height = l - 1
    n_col = int(np.floor((available_width + d_rand) / (d + d_rand)))
    n_row = int(np.floor((available_height + culoar_acces) / (d + culoar_acces)))

    n_col = min(n_col, max_turnuri)  # respectam limita maxima
    n_row = min(n_row, max_turnuri)

    if n_col < 1: n_col = 1
    if n_row < 1: n_row = 1

    x_positions = np.linspace(1, L-1, n_col)
    y_positions = np.linspace(1, l-1, n_row)
...
# --- GENERARE FIGURA ---
fig, total, t_m, c_m, distante_unice = genereaza_plan_tehnic_dinamic(
